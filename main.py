...
# --- حداقل مبلغ برداشت ---
MIN_WITHDRAW_AMOUNT = 1000000  # ۱ میلیون تومان
...
async def process_wth_details(message: types.Message, state: FSMContext):
    ...
    # بررسی مجدد موجودی برای جلوگیری از سوءاستفاده
    user = db.get_user(message.from_user.id, message.from_user.username)
    if user[2] < wth_amount:
        await message.answer("❌ موجودی شما برای انجام این برداشت کافی نیست.")
        await state.clear()
        return

    # ۱. کسر فوری موجودی جهت قفل‌سازی
    db.update_balance(message.from_user.id, -wth_amount)

    # ۲. ثبت تراکنش در دیتابیس
    trans_id = db.add_transaction(message.from_user.id, f"Withdraw_{wth_method}", wth_amount, wth_method)

    # دکمه‌های تایید یا لغو برای ادمین
    markup = types.InlineKeyboardMarkup(inline_keyboard=[[
    ...
    await message.answer("✅ درخواست برداشت شما ثبت شد و مبلغ از موجودی شما کسر گردید.\n⏳ پس از بررسی و تایید مدیریت، مبلغ تسویه خواهد شد.")
    await state.clear()
...
async def approve_withdraw(callback: types.CallbackQuery):
    ...
    # تایید تراکنش (موجودی در زمان ثبت کسر شده است)
    db.update_transaction_status(trans_id, "approved")

    await callback.message.edit_reply_markup(reply_markup=None)
    await callback.message.answer(f"✅ درخواست برداشت #{trans_id} تسویه شد.")
...
async def reject_withdraw(callback: types.CallbackQuery):
    ...
    # بازگرداندن مبلغ به موجودی کاربر در صورت رد درخواست
    db.update_balance(target_uid, amount)
    db.update_transaction_status(trans_id, "rejected")

    await callback.message.edit_reply_markup(reply_markup=None)
    await callback.message.answer(f"❌ درخواست برداشت #{trans_id} لغو و مبلغ **{amount:,.0f} تومان** به موجودی کاربر بازگردانده شد.")
    await bot.send_message(target_uid, f"❌ درخواست برداشت شما به مبلغ **{amount:,.0f} تومان** توسط مدیریت لغو شد و مبلغ به موجودی شما عودت داده شد.")
