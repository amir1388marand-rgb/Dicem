...
async def process_wth_details(message: types.Message, state: FSMContext):
    ...
    # ۱. کسر فوری موجودی کاربر هنگام ثبت درخواست (قفل موجودی)
    ...
    await message.answer("✅ درخواست برداشت شما ثبت شد و مبلغ از حساب شما کسر گردید.\n⏳ پس از بررسی و تایید مدیریت، مبلغ تسویه خواهد شد.")
    await state.clear()
...
async def approve_withdraw(callback: types.CallbackQuery):
    ...
    # تغییر وضعیت تراکنش (موجودی قبلاً کسر شده است)
    db.update_transaction_status(trans_id, "approved")
...
async def reject_withdraw(callback: types.CallbackQuery):
    ...
    # عودت دادن موجودی به حساب کاربر در صورت لغو درخواست
    db.update_balance(target_uid, amount)
    db.update_transaction_status(trans_id, "rejected")

    await callback.message.edit_reply_markup(reply_markup=None)
    await callback.message.answer(f"❌ درخواست برداشت #{trans_id} لغو شد و مبلغ **{amount:,.0f} تومان** به حساب کاربر بازگردانده شد.")
    await bot.send_message(target_uid, f"❌ درخواست برداشت شما به مبلغ **{amount:,.0f} تومان** توسط مدیریت لغو شد و مبلغ به حساب شما عودت داده شد.")
